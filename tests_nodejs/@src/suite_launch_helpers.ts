...
export const runTestDirectly = async (testUrl : string, options : object = {}, inDeno : boolean = false) : Promise<LaunchResult> => {
    return new Promise((resolve, reject) => {
        ...
        child_process.execFile(
            ...
            inDeno
                ?
                    [ 'run', '--allow-read', '--allow-env', '--unstable', '--quiet', testUrl, ...stringifyOptions(options), '--no-color' ]
                :
...
export const runTestViaLauncher = async (testUrl : string, options : object = {}, inDeno : boolean = false) : Promise<LaunchResult> => {
    return new Promise((resolve, reject) => {
        ...
        child_process.execFile(
            ...
            inDeno
                ?
                    [ 'run', '--allow-read', '--allow-env', '--unstable', '--quiet', `deno ${ fileURLToPath(siestaPackageRootUrl) }bin/siesta-deno.js`, testUrl, ...stringifyOptions(options), '--no-color' ]
                :
